 ...
 * 4096x4096, n_tok 1. */
/* The row dimension alone is out_dim/kQ8_0TileRowsPerGroup work-groups of
 * kQ8_0TileGroupSize, so the vocabulary head (129280 rows) crossed with a
 * full prefill chunk (4096 tokens) is billions of work items and exceeds
 * DPC++'s fits-in-int global range. Found by prefilling a 16665-token
 * prompt, which failed outright on this entry. sycl_launch_range_split
 * (ds4_sycl_common.hpp) splits the token dimension so every launch stays
 * inside the limit; a slice is expressed by offsetting the activation and
 * output base pointers, so this kernel is unchanged. Decode passes
 * n_tok == 1 and never splits. */
static sycl::event sycl_q8_0_matmul_tiled_launch(sycl::queue &q, float *out,
    ...
    const sycl::nd_range<2> full(sycl::range<2>((size_t)row_items, n_tok),
                                 sycl::range<2>(kQ8_0TileGroupSize, 1u));
    return sycl_launch_range_split(
            full, /*split_dim=*/1,
            [&](sycl::nd_range<2> chunk, uint64_t tok_base, uint64_t) {
        const float *x_chunk   = x + tok_base * in_dim;
        float       *out_chunk = out + tok_base * out_dim;
        return q.submit([&](sycl::handler &h) {
            ...
            h.parallel_for(
                    ...
                    [=](sycl::nd_item<2> it) [[sycl::reqd_sub_group_size(kQ8_0TileSubgroup)]] {
                        ...
                        /* Stage this token's activation row once; every
                         * lane in the work-group participates in this
                         * strided fill regardless of which output row it
                         * will later compute, so every element up to
                         * in_dim is written by some lane before the
                         * barrier (spec 6b: an idle lane must never be the
                         * reason a local-memory slot goes unwritten). */
...
static sycl::event sycl_q8_0_matmul_blockrow_launch(sycl::queue &q, float *out,
    ...
    const uint64_t row_items = (uint64_t)row_blocks * kQ8_0TileGroupSize;
    /* Same fits-in-int split as sycl_q8_0_matmul_tiled_launch above; see
     * its comment for the shape that first hit the limit. */
    const sycl::nd_range<2> full(sycl::range<2>((size_t)row_items, n_tok),
                                 sycl::range<2>(kQ8_0TileGroupSize, 1u));
    return sycl_launch_range_split(
            full, /*split_dim=*/1,
